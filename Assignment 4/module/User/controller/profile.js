...
const updatUser = async (req,res)=>{
    ...
    const softDelet = async(req,res)=>{
        const id = req.user._id;
        const softDeleted = await userModel.findOneAndUpdate({_id:id} , {deleted : true} , {new : true})
        console.log(softDeleted);
        if (softDeleted) {
            res.json({message:"Gone" , softDeleted})
            } else {
                res.json({message:"email not found"})
            }
    }
    ...
    const allUsers = async(req,res)=>{
            ...
            const userList = await userModel.find({deleted : false}).select("-password")
